...
export function IngredientsListClient({ initialIngredients }: { initialIngredients: Ingredient[] }) {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<string>("All");
  const deferredSearch = useDeferredValue(searchTerm);

  const categories = ["All", "Root / Rhizome", "Leaf / Herb", "Spice / Seed", "Fruit / Berry", "Bark / Wood"];

  const filteredIngredients = useMemo(() => {
    const q = deferredSearch.toLowerCase();
    return initialIngredients.filter((herb) => {
      const matchesSearch =
        !q ||
        herb.commonName.toLowerCase().includes(q) ||
        herb.sanskritName.toLowerCase().includes(q) ||
  ...
  }, [initialIngredients, deferredSearch, selectedCategory]);
    ...
    <div className="bg-[#FAF8F5] min-h-screen py-10 sm:py-14">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="border-b border-[#e2dbcf] pb-8 mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-editorial font-bold text-[#14261B] mt-2 mb-3">Classical Medicinal Plants, Spices & Herbs</h1>
            ...
            >
              <div>
                <div className="h-44 overflow-hidden relative border-b border-[#eee7da]">
                  {herb.featuredImage ? (
                    <Image src={herb.featuredImage} alt={herb.commonName} width={400} height={176} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                  ) : (
                    <div className="w-full h-full bg-[#f0ebe1] flex items-center justify-center text-[#8B6B3E]">
                      <Leaf size={32} />
                    </div>
                  )}
                  <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-xs px-2.5 py-1 rounded text-xs font-semibold text-[#1E4D30]">{herb.sanskritName} ({herb.hindiName})</div>
                  <div className="absolute top-3 right-3 bg-[#14261B]/80 text-white px-2 py-0.5 rounded text-[10px] font-medium">{herb.category}</div>
                </div>
                <div className="p-5">
                  <div className="text-[11px] font-mono text-[#8B6B3E] italic mb-1">{herb.botanicalName}</div>
                  <h3 className="text-xl font-editorial font-bold text-[#14261B] group-hover:text-[#1E4D30] transition-colors leading-snug mb-2">{herb.commonName}</h3>
                  <p className="text-xs sm:text-sm text-[#4d5c50] line-clamp-3 leading-relaxed mb-4">{herb.shortDescription}</p>
