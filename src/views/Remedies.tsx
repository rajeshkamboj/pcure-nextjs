...
export const Remedies: React.FC<RemediesProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    async function loadRemedies() {
      try {
        setIsLoading(true);
        const data = await ContentService.getAllRemedies();
        setRemedies(data);
      } catch (err) {
        console.error('Error fetching remedies:', err);
        setError('Failed to load the remedies library. Please try again later.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#FAF8F5] flex flex-col items-center justify-center p-4">
        ...
        <p className="text-[#4d5c50] font-medium animate-pulse">Loading Desi Nuskhe...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#FAF8F5] flex flex-col items-center justify-center p-4 text-center">
        <div className="bg-white p-8 rounded-xl border border-[#ded5c5] shadow-sm max-w-md">
          <Sparkles size={48} className="mx-auto text-amber-600 mb-4" />
