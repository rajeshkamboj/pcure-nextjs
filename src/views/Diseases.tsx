...
export const Diseases: React.FC<DiseasesProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    async function loadDiseases() {
      try {
        setIsLoading(true);
        const data = await ContentService.getAllDiseases();
        setDiseases(data);
      } catch (err) {
        console.error('Error fetching diseases:', err);
        setError('Failed to load the disease directory. Please try again later.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#FAF8F5] flex flex-col items-center justify-center p-4">
        ...
        <p className="text-[#4d5c50] font-medium animate-pulse">Loading Disease Directory...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#FAF8F5] flex flex-col items-center justify-center p-4 text-center">
        <div className="bg-white p-8 rounded-xl border border-[#ded5c5] shadow-sm max-w-md">
          ...
          <h2 className="text-xl font-bold text-[#14261B] mb-2">Error Loading Directory</h2>
          ...
          <div className="space-y-4">
            {filteredDiseases.map((disease) => (
              <div
                ...
                className="group bg-white rounded-lg border border-[#ded5c5] p-5 sm:p-6 hover:border-[#1E4D30]/60 hover:shadow-md transition-all cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-6"
              >
