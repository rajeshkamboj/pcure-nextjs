...
export const DiseaseDetail: React.FC<DiseaseDetailProps> = ({ slug, onNavigate, initialDisease, initialRemedies, initialIngredients }) => {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        setIsLoading(true);
        const diseaseData = await ContentService.getDiseaseBySlug(slug);

        if (!diseaseData) {
          setError('Disease monograph not found');
          setIsLoading(false);
          return;
        }
        ...
        const [remedies, ingredients] = await Promise.all([
          ContentService.getRemediesForDisease(diseaseData.id),
          ContentService.getIngredientsByIds(diseaseData.keyIngredientsIds)
        ]);

        setAssociatedRemedies(remedies);
        setAssociatedIngredients(ingredients);
      } catch (err) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#FAF8F5] flex flex-col items-center justify-center p-4">
        ...
        <p className="text-[#4d5c50] font-medium animate-pulse">Loading Clinical Monograph...</p>
      </div>
    );
  }
