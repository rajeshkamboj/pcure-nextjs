...
/**
 * How long (seconds) Next.js keeps a WordPress response in its Data Cache.
 * Pages use the same literal for `export const revalidate` (segment configs
...
/*
 * Author lookups are de-duplicated per process/tab, but entries expire so that
 * an edited author profile is not served forever by a long-lived server process.
 */
const AUTHOR_CACHE_TTL_MS = IS_SERVER ? 60 * 1000 : 10 * 60 * 1000;

const authorCache = new Map<
  string,
  { promise: Promise<Author | undefined>; expiresAt: number }
>();
  ...
  const request = (async () => {
    try {
      const res = await fetch(
        `${BASE_URL}/authors/${encodeURIComponent(id)}?_embed`,
        wpFetchInit
      );
...
const normalizeArticle = async (
  ...
  return {
    ...
    coverImageWidth: getFeaturedImageSize(wpArticle).width,

    coverImageHeight: getFeaturedImageSize(wpArticle).height,
...
/*
 * Browser: 10 minutes (fast tab switching).
 * Server: only 30 seconds, purely to de-duplicate requests inside one render
 * burst. Long-lived freshness on the server is handled by the Next.js Data
 * Cache / ISR (`revalidate`), so we must not hide fresh WordPress content
 * behind a second, longer in-memory cache.
 */
const DEFAULT_CACHE_TTL_MS = IS_SERVER ? 30 * 1000 : 10 * 60 * 1000;
...
export const ContentService = {
  ...
  /**
   * Newest `limit` articles only (one small request) — used by the home page,
   * which previously downloaded every article, in full, to show six cards.
   */
  async getLatestArticles(limit: number = 6): Promise<Article[]> {
    const { articles } = await this.getArticlesPaginated(1, limit);
    return articles;
  },
  ...
  /**
   * Returns only the slugs of a post type (`_fields=slug` keeps the response tiny)
   * so detail pages can be pre-rendered at build time. Never throws: if WordPress
   * is unreachable during a build, pages are simply rendered on first request instead.
   */
  async getAllSlugs(
    endpoint: 'diseases' | 'remedies' | 'ingredients' | 'articles'
  ): Promise<string[]> {
    try {
      const data = await fetchAllPages<{ slug?: string }>(endpoint, {
        _fields: 'slug',
      });
      ...
      console.warn(`Unable to list ${endpoint} slugs:`, error);
      return [];
    }
  },
